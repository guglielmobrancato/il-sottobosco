const newsData = [
    {
        "id": 1764019429,
        "date": "24/11/2025",
        "category": "geopolitica",
        "author": "La Redazione",
        "title": "Casa Bianca, 'Trump non \u00e8 stato di parte ai colloqui su Ucraina'",
        "excerpt": "'Il presidente \u00e8 ottimista sul piano per porre fine alla guerra'",
        "body": "\u26a0\ufe0f ERRORE IA: 404 models/gemini-1.5-flash is not found for API version v1beta, or is not supported for generateContent. Call ListModels to see the list of available models and their supported methods.\n\n'Il presidente \u00e8 ottimista sul piano per porre fine alla guerra'",
        "imageIcon": "fa-globe-europe",
        "imageReal": "",
        "link": "https://www.ansa.it/sito/notizie/mondo/nordamerica/2025/11/24/casa-bianca-trump-non-e-stato-di-parte-ai-colloqui-su-ucraina_97adfd9a-4f23-4ecd-9041-38b5b5de880f.html"
    },
    {
        "id": 1764019432,
        "date": "24/11/2025",
        "category": "geopolitica",
        "author": "La Redazione",
        "title": "Brasile, Lula in Mozambico riceve una laurea honoris causa",
        "excerpt": "Il presidente rilancia sulla cooperazione e i biodiesel",
        "body": "\u26a0\ufe0f ERRORE IA: 404 models/gemini-1.5-flash is not found for API version v1beta, or is not supported for generateContent. Call ListModels to see the list of available models and their supported methods.\n\nIl presidente rilancia sulla cooperazione e i biodiesel",
        "imageIcon": "fa-globe-europe",
        "imageReal": "",
        "link": "https://www.ansa.it/sito/notizie/mondo/americalatina/2025/11/24/brasile-lula-in-mozambico-riceve-una-laurea-honoris-causa_9fb057b6-584c-4457-aa0c-aced12990738.html"
    },
    {
        "id": 1764019436,
        "date": "24/11/2025",
        "category": "tech",
        "author": "La Redazione",
        "title": "Apple risarcir\u00e0 proprietari di MacBook per le tastiere difettose",
        "excerpt": "Giudice Usa ha dato via libera all'intesa da 50 milioni di dollari",
        "body": "\u26a0\ufe0f ERRORE IA: 404 models/gemini-1.5-flash is not found for API version v1beta, or is not supported for generateContent. Call ListModels to see the list of available models and their supported methods.\n\nGiudice Usa ha dato via libera all'intesa da 50 milioni di dollari",
        "imageIcon": "fa-microchip",
        "imageReal": "",
        "link": "https://www.ansa.it/sito/notizie/tecnologia/hitech/2023/05/30/apple-risarcira-proprietari-di-macbook-per-le-tastiere-difettose_ce9214c7-b68d-4d8e-ad5f-7bd5bd04ecbe.html"
    },
    {
        "id": 1764019439,
        "date": "24/11/2025",
        "category": "tech",
        "author": "La Redazione",
        "title": "L'inventore di Internet Tim Berners-Lee alla Fiera di Rimini",
        "excerpt": "Ospite l'eccezione a Wmf - We Male Future, dal 15 al 17 giugno",
        "body": "\u26a0\ufe0f ERRORE IA: 404 models/gemini-1.5-flash is not found for API version v1beta, or is not supported for generateContent. Call ListModels to see the list of available models and their supported methods.\n\nOspite l'eccezione a Wmf - We Male Future, dal 15 al 17 giugno",
        "imageIcon": "fa-microchip",
        "imageReal": "",
        "link": "https://www.ansa.it/sito/notizie/tecnologia/internet_social/2023/05/30/linventore-di-internet-tim-berners-lee-alla-fiera-di-rimini_8d846c97-938c-40d3-a479-5100edfc4344.html"
    },
    {
        "id": 1764019443,
        "date": "24/11/2025",
        "category": "cronaca",
        "author": "La Redazione",
        "title": "#AccadeOggi",
        "excerpt": "I principali avvenimenti previsti per la giornata",
        "body": "\u26a0\ufe0f ERRORE IA: 404 models/gemini-1.5-flash is not found for API version v1beta, or is not supported for generateContent. Call ListModels to see the list of available models and their supported methods.\n\nI principali avvenimenti previsti per la giornata",
        "imageIcon": "fa-user-secret",
        "imageReal": "",
        "link": "https://www.ansa.it/sito/notizie/cronaca/2025/11/24/accadeoggi_dd38a7d3-309c-4e12-a6b9-d7b1590e0cf6.html"
    },
    {
        "id": 1764019446,
        "date": "24/11/2025",
        "category": "cronaca",
        "author": "La Redazione",
        "title": "Decaro, \u00e8 un risultato elettorale straordinario",
        "excerpt": "'Stasera festeggio, domani mi metter\u00f2 a lavorare per i pugliesi'",
        "body": "\u26a0\ufe0f ERRORE IA: 404 models/gemini-1.5-flash is not found for API version v1beta, or is not supported for generateContent. Call ListModels to see the list of available models and their supported methods.\n\n'Stasera festeggio, domani mi metter\u00f2 a lavorare per i pugliesi'",
        "imageIcon": "fa-user-secret",
        "imageReal": "",
        "link": "https://www.ansa.it/sito/notizie/cronaca/2025/11/24/decaro-e-un-risultato-elettorale-straordinario_8b2ff5da-8522-463e-bb65-2c5d6d2039f6.html"
    }
];